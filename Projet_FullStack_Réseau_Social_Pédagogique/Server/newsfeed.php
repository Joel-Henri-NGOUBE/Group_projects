<?php 
$friends = FILTER_INPUT(INPUT_POST,"body");
...
foreach ($friends as $ami){
    $request2 = $pdo->prepare("SELECT username, publications.*,DATE(publications.date) AS jour,DATE_FORMAT(publications.date,'%Hh%i') AS heure,DATEDIFF(CURRENT_TIMESTAMP,publications.date) AS marge FROM users INNER JOIN publications ON users.id = publications.id_user WHERE id_user = :id AND DATEDIFF(CURRENT_TIMESTAMP,publications.date) <= 7;");
    $request2->execute([
        ":id" => $ami["friend_id"]
...
echo json_encode(["Statut" => "Succès", "Message" => "Aucun ami n'a publié récemment"]);
}
